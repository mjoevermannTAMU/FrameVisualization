...
class EasyBox:
    def __init__(self, center,rotation_center, size, rotation, ax="none", color=(0.3, 0.3, 1, 1), name="[unnamed box]"):
        self.center = center
    ...
    def update_position_and_rotation(self):
        ...
        # update to rotations
        box_x, box_y, box_z = np.dot(np.array([box_x, box_y, box_z]).T - np.array([self.rotation_center.x, self.rotation_center.y, self.rotation_center.z]), rotation_matrix_x).T
...
class EasySphere:
    ...
    def __init__(self, center, radius, rotation, ax="none", color="none", edgecolor = (0.1,0.1,0.1,0.2), name="[unnamed sphere]", plot_density = [30,30]):
        self.center = center
        self.radius = radius
        self.rotation = rotation
...
#     print("updating", model.boxes[0].rotation.x)
#     model.boxes[0].rotation.x = x_rotation_slider.val
#     for box in model.boxes:
#         print("new value:", box.rotation)
#
...
sphere = model.add_sphere(EasySphere(center=Vector3(0, 0, 0), rotation=Vector3(0,0,0), radius=12, name = "shell"))
pendulum1 = model.add_box(EasyBox(center= Vector3(0, 0, 0), rotation=Vector3(0,0,0), rotation_center=Vector3(0,0,1.5), size = Vector3(2,2,2), color=(1,0.3,0.3,0.5), name="pendulum 1"))
pendulum2 = model.add_box(EasyBox(center= Vector3(0, 0, 0), rotation=Vector3(0,0,0), rotation_center=Vector3(0,0,5), size = Vector3(2,2,5), color=(1,0.3,0.3,0.5)))
pendulum3 = model.add_box(EasyBox(center= Vector3(0, 5, 0), rotation=Vector3(0,0,0), rotation_center=Vector3(0,0,0), size = Vector3(2,2,5), color=(1,0.3,0.3,0.5)))
...
x_rotation, y_rotation, z_rotation, a, b = 0, 0, 0, 0, 0
class SliderWindow:
    ...
    x_rotation = tk.DoubleVar()
    y_rotation = tk.DoubleVar()
    z_rotation = tk.DoubleVar()
    a = tk.DoubleVar()
    b = tk.DoubleVar()

    val = tk.Scale(sliderWindow, from_=-179, to=-91, variable=x_rotation, orient='horizontal',
                   label='Set Elevation Angle (deg)', length=300, command=test)
    val.set(-160)  # initialize slider value
    val.pack()
    val = tk.Scale(sliderWindow, from_=-180, to=180, variable=y_rotation, orient='horizontal',
                   label='Set Azimuth Angle (deg)', length=300, command=test)
    val.pack()
    val = tk.Scale(sliderWindow, from_=-90, to=90, variable=z_rotation, orient='horizontal',
                   label='Set Yaw Angle (deg)', length=300, command=test)
    val.pack()
    val = tk.Scale(sliderWindow, from_=-90, to=90, variable=a, orient='horizontal', label='Set Drive Angle (deg)',
                   length=300, command=test)
    val.pack()
    val = tk.Scale(sliderWindow, from_=-60, to=60, variable=b, orient='horizontal', label='Set Steer Angle (deg)',
                   length=300, command=test)
    val.pack()
    B = tk.Button(sliderWindow, text="Stop Program", command=test)
    B.pack()
