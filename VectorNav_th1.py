...
class Angles:
    ...
    def update_th2(self, x):
        self.theta_2 = x
        print(f'Theta 2 has changed to {self.theta_2}')
...
# now apply a lamprey rotation angle about its z axis
def update_plots(angle):
    # calculate the orientation of the pitch frame, f1
    f1_pose =  R_init @ Rz(-float(angle)*np.pi/180) @ Rx(np.pi/2)
...
val = tk.Scale(sliderWindow, from_=-60, to=60, variable=encoder_value, orient='horizontal', label='Set Rotation (deg)', length=300, command=update_plots)
val.pack()
